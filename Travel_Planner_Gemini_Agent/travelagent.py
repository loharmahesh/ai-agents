...
# Step 2: Dropdown for departure and destination
st.markdown("### 🌍 Where are you headed?")

source_city = st.selectbox("🛫 Departure City:", options=list(iata_options.keys()))
destination_city = st.selectbox("🛬 Destination City:", options=list(iata_options.keys()), index=1)

# Step 3: Extract IATA code
source = iata_options[source_city]
destination = iata_options[destination_city]
